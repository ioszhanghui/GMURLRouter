...
+ (void)pushViewController:(UIViewController *)viewController query:(NSDictionary* __nullable)query animated:(BOOL)animated{
    [GMURLNavgation pushViewController:viewController query:query animated:animated];
}
...
+(void)pushViewControllerWithName:(NSString*)VCName Animated:(BOOL)animated{
    ...
    [GMURLRouter pushViewController:[VC new] Animated:animated];
}
...
+ (void)pushURLString:(NSString *)urlString query:(NSDictionary *)query animated:(BOOL)animated callBack:(void (^)(id result))callBack{
    UIViewController * viewController = [UIViewController initFromString:urlString withQuery:query fromConfig:[GMURLRouter sharedGMURLRouter].cacheConfigDict callBack:callBack];
    [GMURLNavgation pushViewController:viewController query:query callBack:callBack animated:animated];
}
...
+ (void)presentViewController:(UIViewController *)viewController animated: (BOOL)flag{
    [GMURLNavgation presentViewController:viewController animated:flag];
}
/*present页面 参数query*/
+ (void)presentViewController:(UIViewController *)viewController query:(NSDictionary* __nullable)param animated: (BOOL)flag{
    [GMURLNavgation presentViewController:viewController query:param animated:flag];
}
...
+ (void)presentViewController:(UIViewController *)viewController query:(NSDictionary* __nullable)param animated: (BOOL)flag completion:(void (^ __nullable)(id result))callback{
    [GMURLNavgation presentViewController:viewController query:param animated:flag completion:callback];
}
/*present页面 urlString 自定义URL,也可以拼接参数,但会被下面的query替换掉  query     存放参数  回调callback*/
+ (void)presentURLString:(NSString *)urlString query:(NSDictionary *)query animated:(BOOL)animated completion:(void (^ __nullable)(id result))callBack;{
    UIViewController * viewController = [UIViewController initFromString:urlString withQuery:query fromConfig:[GMURLRouter sharedGMURLRouter].cacheConfigDict  callBack:callBack];
    [GMURLNavgation presentViewController:viewController query:query animated:animated completion:callBack];
}
